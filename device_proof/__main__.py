...
def _export_bundle(argv):
    ...
    parsed = parser.parse_args(argv)
    try:
        bundle_doc = export_bundle(
            parsed.job_id, parsed.output, runtime_root=parsed.runtime_dir)
    ...
    print(json.dumps({
        ...
        "bundle_version": bundle_doc["bundle_version"],
...
def _verify_bundle(argv):
    parser = argparse.ArgumentParser(
        ...
        description="Verify an exported evidence bundle offline and print the "
